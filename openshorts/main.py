...
def download_youtube_video(url, output_dir="."):
    ...
    if cookies_env:
        ...
        # Local patch: fall back to a cookies.txt bind-mounted via the repo
        # root (/app is the bind mount), so the full browser export works
        # without pushing 1MB of cookie text through an env var.
        print("🍪 Using existing /app/cookies.txt (bind-mounted from host).")
    else:
        cookies_path = None
        print("⚠️ YOUTUBE_COOKIES env var not found and no /app/cookies.txt present.")
    ...
    # Optional source-resolution cap (MAX_SOURCE_HEIGHT, e.g. "720"): applies to
    # EVERY attempt including the fallback, which is what keeps Twitch's
    # 1080p60 HLS from wedging on a flaky network (the 720p rendition pulls in a
    # fraction of the bytes; reframe_v2.delivery_size upscales the 9:16 crop
    # back to 1080-wide, so finished clips are still 1080x1920). Deployment
    # default via .env, or per job via the API's max_source_height field.
    _raw_cap = os.environ.get("MAX_SOURCE_HEIGHT", "").strip()
    try:
        _source_cap = int(_raw_cap) if _raw_cap else None
    except ValueError:
        _source_cap = None
        print(f"⚠️ MAX_SOURCE_HEIGHT={_raw_cap!r} is not an integer — ignoring.")
    if _source_cap is not None and _source_cap < 144:
        print(f"⚠️ MAX_SOURCE_HEIGHT={_source_cap} is below 144p — ignoring.")
        _source_cap = None
    if _source_cap:
        print(f"📐 MAX_SOURCE_HEIGHT={_source_cap}: capping the source at {_source_cap}p.")

    def _hd_fmt_for(capped):
        if _source_cap:
            cap = _source_cap
            return (f'bestvideo[vcodec^=avc1][height<={cap}][ext=mp4]+bestaudio[ext=m4a]/'
                    f'bestvideo[vcodec^=avc1][height<={cap}]+bestaudio/'
                    f'best[height<={cap}][ext=mp4]/best[height<={cap}]/best')
        if capped:
    ...
    fallback_fmt = (f'best[height<={_source_cap}][ext=mp4]/best[height<={_source_cap}]/best'
                    if _source_cap else 'best[ext=mp4]/best')
