...
- REWATCH VALUE: moments that stay funny/interesting on a second watch outrank
  one-shot surprises — rewatch rate is the strongest reach multiplier on
  TikTok and Shorts in 2026.
...
- LOOP RULE: the clip must end within ~1 second of the payoff — trim trailing
  silence, filler words, and "so yeah"-style wind-downs. An ending that snaps
  back naturally toward the opening beat earns instant rewatches, and rewatch
  rate is the strongest reach multiplier on TikTok and Shorts.
...
HOOK PLAYBOOK — pick the strongest fitting pattern for `viral_hook_text`
(5-8 words is the sweet spot, 10 max; create a curiosity gap, never spoil
the payoff):
