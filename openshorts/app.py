...
async def process_endpoint(
    ...
    max_source_height: Optional[str] = Form(None),
):
    ...
    if "application/json" in content_type:
        ...
        max_source_height = body.get("max_source_height")
    ...
    # Per-job cap on the SOURCE download resolution (main.py reads
    # MAX_SOURCE_HEIGHT): Twitch's 1080p60 HLS wedges on flaky networks and the
    # 9:16 delivery upscales to 1080-wide either way, so 720 costs little.
    # Absent → the deployment default (.env) applies; an explicit empty string
    # opts this job back out of it; bad values 400 instead of silently
    # downloading something the caller didn't ask for.
    if max_source_height is not None:
        raw_height = str(max_source_height).strip()
        if raw_height == "":
            env.pop("MAX_SOURCE_HEIGHT", None)
        else:
            h_cap = _gen_control(raw_height, "max_source_height", 144, 2160, integer=True)
            env["MAX_SOURCE_HEIGHT"] = str(h_cap)
            print(f"[source-cap] job={job_id} max_source_height={h_cap}")
